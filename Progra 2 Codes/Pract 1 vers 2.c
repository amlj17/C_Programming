...
//  
//
//  Created by Andres Moguel on 2/22/21.
//
#include<stdio.h>
#include <stdlib.h>
#include <string.h>
...
//Funciones Prototipo
unsigned char ** LeerArchivo(char NombreArchivo[100], int *Columnas, int *Filas);
...
int main(void)
{
  ...
  //Mensajes de bienvenida y creacion
    ...
    Mat_Imagen = LeerArchivo(NombreArchivo,&Columnas,&Filas);
...
void mostrar_instrucciones(void){
    ...
    printf("\n---Presiona la tecla Enter para comenzar a usar el programa---\n");
    ...
    if(Archivo != NULL){
        ...
        for(i=0;i<*Filas;i++){
            for(j=0;j<*Columnas;j++){
                fscanf(Archivo, "%hhu",&Mat_Imagen[i][j]);
            }
        }
        fclose(Archivo);
...
void Ecualizar(unsigned char **Mat_Imagen, int Columnas, int Filas){
    ...
    //encontramos b
    b=(-1)*(pendiente*Maximo)
    //ecualizamos la imagen
    for(i=0;i<Filas;i++){
        for(j=0;j<Columnas;j++){
            Nueva[i][j]=((pendiente*Mat_Imagen[i][j])+b);
        }
    }
