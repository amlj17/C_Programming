...
void menu(void);
void lee_archivo(tipo_alumno lista[],int *posicion, int total_alumnos);
...
int main(void)
{
    ...
    do
    {
        ...
        switch (opcion)
        {
            case 1: if(posicion<total_alumnos)
                        lee(grupo,&posicion);
                    else
            ...
            case 5: guarda_archivo(grupo,posicion);
                    break;
            case 6: free(grupo);
                lee_archivo(&grupo,&posicion, &total_alumnos);
                    break;
            case 7: printf("Adios\n");
                    break;
            default:
...
void lee_archivo(tipo_alumno **lista,int * posicion, int * total_alumnos)
{
    ...
    if (archivo!=NULL)
    {
        ...
        *lista = (tipo_alumno*)malloc(sizeof(tipo_alumno)*tamanobytes);
